...
import { RoutineCase, RoutineCaseService } from '../routine-case';
...
export class AppointmentDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.routineCaseService
            .query({filter: 'appointment-is-null'})
            .subscribe((res: ResponseWrapper) => {
                if (!this.appointment.routineCase || !this.appointment.routineCase.id) {
                    this.routinecases = res.json;
                } else {
                    this.routineCaseService
                        .find(this.appointment.routineCase.id)
                        .subscribe((subRes: RoutineCase) => {
                            this.routinecases = [subRes].concat(res.json);
    ...
    trackRoutineCaseById(index: number, item: RoutineCase) {
        return item.id;
    }
}
