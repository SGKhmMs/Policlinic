...
public class ClinicDoctorResource {
    ...
    @Timed
    public List<ClinicDoctor> getAllClinicDoctors(@RequestParam(required = false) String filter) {
        if ("clinic-is-null".equals(filter)) {
            log.debug("REST request to get all ClinicDoctors where clinic is null");
            return StreamSupport
                .stream(clinicDoctorRepository.findAll().spliterator(), false)
                .filter(clinicDoctor -> clinicDoctor.getClinic() == null)
                .collect(Collectors.toList());
        }
        if ("doctor-is-null".equals(filter)) {
            log.debug("REST request to get all ClinicDoctors where doctor is null");
            return StreamSupport
                .stream(clinicDoctorRepository.findAll().spliterator(), false)
                .filter(clinicDoctor -> clinicDoctor.getDoctor() == null)
                .collect(Collectors.toList());
        }
