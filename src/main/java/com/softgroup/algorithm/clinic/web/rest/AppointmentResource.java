...
public class AppointmentResource {
    ...
    @Timed
    public List<Appointment> getAllAppointments(@RequestParam(required = false) String filter) {
        if ("doctor-is-null".equals(filter)) {
            log.debug("REST request to get all Appointments where doctor is null");
        ...
        if ("client-is-null".equals(filter)) {
            log.debug("REST request to get all Appointments where client is null");
            return StreamSupport
                .stream(appointmentRepository.findAll().spliterator(), false)
                .filter(appointment -> appointment.getClient() == null)
                .collect(Collectors.toList());
        }
        if ("serviceonappointment-is-null".equals(filter)) {
            log.debug("REST request to get all Appointments where serviceOnAppointment is null");
            return StreamSupport
                .stream(appointmentRepository.findAll().spliterator(), false)
                .filter(appointment -> appointment.getServiceOnAppointment() == null)
                .collect(Collectors.toList());
        }
