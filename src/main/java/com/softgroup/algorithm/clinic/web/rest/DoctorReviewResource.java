...
public class DoctorReviewResource {
    ...
    @Timed
    public List<DoctorReview> getAllDoctorReviews(@RequestParam(required = false) String filter) {
        if ("appointment-is-null".equals(filter)) {
            log.debug("REST request to get all DoctorReviews where appointment is null");
            return StreamSupport
                .stream(doctorReviewRepository.findAll().spliterator(), false)
                .filter(doctorReview -> doctorReview.getAppointment() == null)
