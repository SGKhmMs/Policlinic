...
public class CardEntryResource {
    ...
    @Timed
    public List<CardEntry> getAllCardEntries(@RequestParam(required = false) String filter) {
        if ("appointment-is-null".equals(filter)) {
            log.debug("REST request to get all CardEntrys where appointment is null");
            return StreamSupport
                .stream(cardEntryRepository.findAll().spliterator(), false)
                .filter(cardEntry -> cardEntry.getAppointment() == null)
