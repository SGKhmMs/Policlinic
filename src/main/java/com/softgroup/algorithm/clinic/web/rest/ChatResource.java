...
import com.softgroup.algorithm.clinic.repository.ChatRepository;
...
public class ChatResource {
    ...
    public List<Chat> getAllChats(@RequestParam(required = false) String filter) {
        if ("doctor-is-null".equals(filter)) {
            log.debug("REST request to get all Chats where doctor is null");
            return StreamSupport
                .stream(chatRepository.findAll().spliterator(), false)
                .filter(chat -> chat.getDoctor() == null)
                .collect(Collectors.toList());
        }
        if ("client-is-null".equals(filter)) {
            log.debug("REST request to get all Chats where client is null");
            return StreamSupport
                .stream(chatRepository.findAll().spliterator(), false)
                .filter(chat -> chat.getClient() == null)
                .collect(Collectors.toList());
        }
